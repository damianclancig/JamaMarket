...
export async function generateMetadata(): Promise<Metadata> {
    ...
    return {
        title: t('My_Orders'),
        description: t('Orders_Meta_Desc'),
        robots: {
            index: false,
            follow: false,
        }
...
function StatusAlert({ status, t }: { status: string | undefined, t: (key: keyof typeof translations) => string }) {
    if (status === 'success') {
        return (
            <Alert variant="default" className="bg-green-100 dark:bg-green-900 border-green-500 text-green-800 dark:text-green-200 mb-6">
                ...
                <AlertTitle className="font-bold">{t('Payment_Approved_Title')}</AlertTitle>
                <AlertDescription>
                    {t('Payment_Approved_Desc')}
                </AlertDescription>
            </Alert>
        );
    }
    ...
    if (status === 'failure') {
        return (
            <Alert variant="destructive" className="mb-6">
                ...
                <AlertTitle>{t('Payment_Failed_Title')}</AlertTitle>
                <AlertDescription>
    ...
    if (status === 'pending') {
        return (
            <Alert className="mb-6">
                ...
                <AlertTitle>{t('Payment_Pending_Title')}</AlertTitle>
                <AlertDescription>
...
function PaymentDetails({ details, t }: { details: MercadoPagoPaymentDetails, t: (key: keyof typeof translations) => string }) {
    ...
    return (
        <Card className="mb-6 bg-muted/50">
            <CardHeader>
                ...
                <CardDescription>{t('MercadoPago_Details')}</CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-2 gap-4 text-sm">
                <div>
                    <p className="font-semibold">{t('Payment_Method')}</p>
                    <p className="capitalize">{formatPaymentMethod(details.paymentMethodId)}</p>
                </div>
                <div>
                    <p className="font-semibold">{t('Payment_Type')}</p>
                    <p className="capitalize">{formatPaymentMethod(details.paymentTypeId)}</p>
                </div>
                {details.lastFourDigits && (
                    <div>
                        <p className="font-semibold">{t('Card')}</p>
                        <p>•••• {details.lastFourDigits}</p>
                    </div>
                )}
                {details.installments && (
                    <div>
                        <p className="font-semibold">{t('Installments')}</p>
...
async function OrdersContent({ searchParams }: { searchParams: { status?: string } }) {
    ...
    return (
        <div className="container py-8 md:py-12">
            ...
            <StatusAlert status={searchParams.status} t={t} />
                                    ...
                                    <div className="mb-6 space-y-4">
                                        {order.paymentMethod === 'Efectivo' && order.status === 'Pendiente' && (
                                            <Alert>
                                                ...
                                                <AlertTitle>{t('Cash_Payment')}</AlertTitle>
                                                <AlertDescription>
                                        ...
                                        {order.status === 'Pendiente de Confirmación' && order.receiptUrl && (
                                            <Card className="bg-muted/50">
                                                <CardHeader>
                                                    <CardTitle className="text-lg">{t('Receipt_Sent')}</CardTitle>
                                                    <CardDescription>{t('Receipt_Sent_Desc')}</CardDescription>
                                                </CardHeader>
                                                <CardContent>
                                        ...
                                        {order.paymentMethod === 'MercadoPago' && order.mercadoPagoPaymentDetails && order.status !== 'Pendiente de Pago' && (
                                            <PaymentDetails details={order.mercadoPagoPaymentDetails} t={t} />
                                        )}
                                    </div>
...
export default async function OrdersPage(props: { searchParams: Promise<{ status?: string }> }) {
    ...
    return (
        <Suspense fallback={<div className="container py-8">{t('Loading_Orders')}</div>}>
