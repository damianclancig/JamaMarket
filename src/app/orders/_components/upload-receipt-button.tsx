...
export default function UploadReceiptButton({ orderId }: UploadReceiptButtonProps) {
  ...
  const handleSubmit = () => {
    if (!imageUrl) {
        toast({ variant: 'destructive', title: t('Error_Title'), description: t('Upload_Image_First') });
        return;
    }
    startSubmitTransition(async () => {
        const result = await submitReceipt(orderId, imageUrl);
        if (result.success) {
            toast({ title: t('Receipt_Sent'), description: t('Receipt_Sent_Desc') });
            await refreshPendingCount();
            setOpen(false);
            router.refresh();
        } else {
            toast({ variant: 'destructive', title: t('Error_Title'), description: result.message });
        }
    });
  }
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{t('Upload_Receipt_Title')}</AlertDialogTitle>
          <AlertDialogDescription>
        ...
        <div className="space-y-4">
            {imageUrl ? (
                 <div className="relative aspect-video w-full rounded-md overflow-hidden">
                    <NextImage src={imageUrl} alt={t('Image')} fill className="object-contain" />
                 </div>
            ) : (
                <div className="relative aspect-video w-full border-2 border-dashed rounded-md flex flex-col items-center justify-center text-muted-foreground">
                    {loading ? (
                         <div className="flex flex-col items-center gap-2 text-center">
                            ...
                            <p className="text-sm">{t('Uploading_Ellipsis')}</p>
                        </div>
                    ) : (
