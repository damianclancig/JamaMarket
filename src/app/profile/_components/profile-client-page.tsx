...
function ProfileImageUploader({ currentImage, onImageUpload, onImageRemove, isSubmitting }: { 
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setLoading(true);
        toast({ title: t('Uploading_Image_Title'), description: t('Uploading_Image_Desc') });
        ...
        try {
            ...
            toast({ title: t('Image_upload_success_title'), description: t('Profile_image_updated_desc') });
        } catch (err: any) {
    ...
    return (
        <div className="flex flex-col items-center gap-4">
            <div className="relative h-32 w-32 rounded-full overflow-hidden bg-muted border-4 border-background shadow-lg">
                {currentImage ? (
                    <Image src={currentImage} alt={t('Profile_Photo')} fill className="object-cover" />
                ) : (
