...
export default function MultiMediaUploader({ name, defaultValues = [] }: MultiMediaUploaderProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast({
      ...
      description: t('Uploading_File_Count', { count: files.length }),
    });
      ...
      try {
        ...
        const res = await fetch(
          ...
          if (!res.ok) {
            ...
            throw new Error(errorData.error.message || t('Upload_Failed', { name: file.name }));
          }
    ...
    toast({
      ...
      description: t('Upload_Success_Count', { count: uploadedUrls.length }),
    });
          ...
          <div key={index} className="relative aspect-square w-full rounded-md overflow-hidden">
            {isVideo(url) ? (
                ...
                <NextImage
                  ...
                  alt={t('Media_Preview_Alt', { index: index + 1 })}
                  fill
            ...
            <div className="absolute top-1 right-1 z-10">
              ...
              >
                <X className="h-4 w-4" />
                <span className="sr-only">{t('Delete_Media')}</span>
              </Button>
            </div>
             {isVideo(url) && (
